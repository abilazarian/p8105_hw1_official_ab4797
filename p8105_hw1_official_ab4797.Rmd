...
```{r}
(as.numeric(pull(hw_problem1_df,vec_logical)) * (pull(hw_problem1_df,samp)))
```
...
```
Here we can convert the logical vector to a factor but we cannot multiply it by the result. Below is the code to convert without the multiplication step. Here is the code multiplying the vector by the result that result in an error: (vec_logical_factor)*((pull(hw_problem1_df,samp))
